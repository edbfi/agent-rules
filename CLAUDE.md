...
Stdlib-only Python (3.11+ for `tomllib`); nothing to install. Run these checks locally:
...
Review changes left by the checks with `git diff`.
...
- Opting a repo in is a manifest-only change (see commit `3e39eb1`). Delivery workflows are disabled.
...
- Delivery workflows are disabled; review rule changes and copy them manually.
